...
public class TheRegister {
    ...
    public void registerDomains() {
        ...
        // endpoints to test the pattern path
        webFramework.registerPatternPath(GET, Pattern.compile("patternpath/(\\d+)"), (request, matcher) -> Response.htmlOk("Number: " + matcher.group(1)));
        webFramework.registerPatternPath(GET, Pattern.compile("patternpath/range/(?<from>\\d+)-(?<to>\\d+)"), (request, matcher) -> {
